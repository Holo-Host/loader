...
export default class HoloResolver {
  ...
  _UIHost = '' // Currently queried host

  constructor (location) {
    this._url = location.hostname
  }

  getIframeAddress = bundleHash => this.getHost().then(this.formatAddress)
  ...
  queryForHosts = (bundleHash = '') => {
    const url = this._url
    const requestBody = `url=${encodeURIComponent(url)}&hash=${encodeURIComponent(
      bundleHash
    )}`
